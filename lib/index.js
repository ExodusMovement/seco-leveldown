...
SecoDOWN.prototype._open = function (opts, cb) {
  ...
  callback(_asyncToGenerator(function* () {
    _this._seco = (0, _secoRw2.default)(_this.location, opts.passphrase || 'Hi', opts.header);
    if (!(yield (0, _pathExists2.default)(_this.location))) {
      yield _this._seco.write('{}');
      _this._data = {};
    } else _this._data = JSON.parse((yield _this._seco.read()));
  }), cb);
};
...
SecoDOWN.prototype._write = _asyncToGenerator(function* () {
  ...
  yield this._writeQueue.add(function () {
    return _this3._seco.write(JSON.stringify(_this3._data));
  });
});
